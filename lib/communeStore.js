...
function loadCodePostaux() {
  ...
  function addCodePostalToCommune(codePostal, codeInsee) {
    ...
    if (cpIndex.has(codePostal)) {
      cpIndex.get(codePostal).add(commune);
    } else {
      cpIndex.set(codePostal, new Set([commune]));
    }
    commune.codesPostaux.add(codePostal);
    count++;
  }
...
function convertSets() {
  ...
  cpIndex.forEach((communes, codePostal) => {
    cpIndex.set(codePostal, Array.from(communes));
  });
}

module.exports = function () {
  return loadCommunes().then(loadCodePostaux).then(convertSets).then(() => ({
    query: (lon, lat) => {
    ...
    queryByCodeInsee: codeInsee => {
      return communes[codeInsee];
    }
  }));
};

module.exports.normalizeNom = normalizeNom;
