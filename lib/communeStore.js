...
module.exports = function () {
  return loadCommunes().then(loadCodePostaux).then(() => ({
    query: (lon, lat) => index.query(lon, lat),
    queryByName: nom => textIndex.search(nom).map(result => {
      const commune = pick(communes[result.ref], 'codeInsee', 'nom', 'codesPostaux', 'centre', 'surface');
      commune.score = result.score;
      return commune;
    }),
    queryByCP: codePostal => {
      if (cpIndex.has(codePostal)) {
        return cpIndex.get(codePostal).map(commune => pick(commune, 'codeInsee', 'nom', 'codesPostaux', 'centre', 'surface'));
      } else {
        return [];
      }
    }
