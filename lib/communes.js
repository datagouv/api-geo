...
function getIndexedDb(options = {}) {
  ...
  communes.forEach(commune => {
    ...
    commune.codesPostaux.forEach(codePostal => {
      ...
      let depNb = codePostal.substring(0,2);
      if (codeDep.has(depNb)) {
        let dep = codeDep.get(depNb)[0];
        if (dep.nom !== commune.nom) { // Avoid duplicating communes like Paris
          codeDep.get(depNb).push(commune);
        }
      } else {
        codeDep.set(depNb, [commune]);
      }
  ...
  function queryByDep(codeDepartement) {
    if (codeDep.has(codeDepartement)) {
      return codeDep.get(codeDepartement);
    } else {
      return [];
    }
  }
