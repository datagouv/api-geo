...
function init(ctx, next) {
  ...
  ctx.createCommune = code => {
    if (ctx.communes.has(code)) {
      throw new Error('La commune est déjà dans le référentiel')
    }
...
function loadCommunes(options = {}) {
  return function (ctx, next) {
    ...
    fs.createReadStream(options.srcPath || __dirname + '/../../data/France2018.txt')
      ...
      .on('error', next)
      .pipe(t((data, enc, cb) => {
        if (!['1', '2', '3', '5', '6'].includes(data.ACTUAL)) {
          return cb()
        }

        if (data.ACTUAL === '3' && !data.POLE) {
          return cb()
        }

        const code = data.DEP + data.COM
        if (isIgnoredCommune(code)) {
          return cb()
        }
        ...
        } else if (data.ACTUAL === '5') {
          commune.type = 'arrondissement-municipal'
          commune.codeRegion = data.REG
          commune.codeDepartement = data.DEP === '97' ? data.DEP + data.COM.charAt(0) : data.DEP
        } else {
          commune.type = 'commune-ancienne'
    ...
    ctx.communes.forEach(commune => {
      if (!commune.ancienne && commune.communesMembres.size > 0) {
        ...
        commune.communesMembres.forEach(communeMembre => {
          if (communeMembre.contour) {
            communeContours.push(communeMembre)
          }
        })
        if (communeContours.length > 0) {
          if (commune.contour) {
            communeContours.push(commune)
          }
        ...
        commune.communesMembres.forEach(communeMembre => {
          if (communeMembre.population) {
            communePopulations.push(communeMembre)
          }
        })
        if (communePopulations.length > 0) {
          if (commune.population) {
            communePopulations.push(commune)
          }
